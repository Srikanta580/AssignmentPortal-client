...
import { fetchStudents } from "../../features/admin/adminAPI";

const Attendance = () => {
  const { classId, semId } = useParams();
  const { classes } = useSelector((state) => state?.faculty);
  const { id: universityId } = useSelector((state) => state.university);
  const { department } = useSelector((state) => state.auth?.user);
  const dispatch = useDispatch();

  const currentClass = classes.find(
    (cls) => cls.id.toString() === classId.toString()
  );
  ...
  useEffect(() => {
    ...
    fetchClassStudents();
  }, [dispatch, classId, universityId, semId, department.id]);
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-xs text-[#495057] mt-1 flex items-center">
                ...
                {currentClass.timing} • Room {currentClass.location}
              </div>
            </div>
            <div className="text-right">
              <div className="text-xs font-medium text-[#005f73]">
                {students.length > 0 ? currentIndex + 1 / students.length : 0}
              </div>
